import { Directive, ComponentFactoryResolver, ViewContainerRef, Type, ComponentRef, Input, OnInit, Output, EventEmitter, OnChanges, SimpleChanges, ChangeDetectionStrategy } from '@angular/core';
...
export class CnFormItemDirective implements OnInit, OnChanges {
  @Input() public config;
  ...
  public ngOnInit() {
    ...
    if (this.config.state === 'text') {
      _config = this.config.text;
    }
    if (this.config.state === 'edit') {
      _config = this.config.editor;
    }
