...
@Component({
  selector: 'app-cn-grid-tree-select',
  templateUrl: './cn-grid-tree-select.component.html',
  styleUrls: ['./cn-grid-tree-select.component.less']
})
export class CnGridTreeSelectComponent  extends CnComponentBase implements OnInit {
  @Input() public config;
  @Input() tempData;
  @Input() initData;
  ...
  async valueChange(v: string): Promise<void> {
   ...
   const backValue = { name: this.config.field, value: v, id:  this.valueConfig.id,count: this.count, dataItem: item };
